...
const FormView = () => {
  ...
  const fetchHeader = async () => {
    try {
      ...
      setColumnVisibility(
        Object.fromEntries(
          transformedColumns.map((column) => [column.accessor, true])
        )
      );
    } catch (error) {
  ...
  useEffect(() => {
    if (getFormcode || getTableName) {
      fetchData(getFormcode, getTableName);
    }
  }, [pageSize, currentPage, filterStatus, getFormcode, getTableName]);
              ...
              <div className="col-md-8 d-flex justify-content-end align-items-center">
                <div className="btn-group ml-2">
                  ...
                  <button
                    type="button"
                    className={`btn ${
                      isCustomizeOpen ? "btn-secondary" : "btn-default"
                    }`}
