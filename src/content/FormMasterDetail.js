...
const FormMasterDetail = () => {
  ...
  const fetchHeader = async () => {
    ...
    try {
      ...
      const formCode = response.data.coreFields.map(
        (getFormcode) => getFormcode.formCode
      );
      ...
      if (response.data.tableDetail !== null) {
        console.log("Log Point: Set Table Detail", response.data.tableDetail);
        setTableNameDetail(response.data.tableDetail);
      } else if (response.data.tableDetail === null) {
        console.log(
          "Log Point: Table Detail is Null",
          response.data.tableDetail
        );
        setTableNameDetail("");
      }
  ...
  const exportToCSV = () => {
    ...
    XLSX.writeFile(wb, `${menuName}-${getCurrentDateTime()}.csv`, {
      bookType: "csv",
    }); // Note the added bookType parameter for CSV
  };
  const downloadFileCsv = async () => {
    try {
      // Fetch the data from the API
      const response = await axios.get(
        `${FORM_SERVICE_REPORT_DATA_CSV}?formId=${idForm}`,
        {
          headers: headers,
  ...
  const downloadFileXls = async () => {
    try {
      // Fetch the data from the API
      const response = await axios.get(
        `${FORM_SERVICE_REPORT_DATA_EXCEL}?formId=${idForm}`,
        {
          headers: headers,
          responseType: "blob", // Set the response type to 'blob' to handle binary data
        }
              ...
              <div className="col-md-8 d-flex justify-content-end align-items-center">
                <div className="btn-group ml-2">
                  ...
                  <div className="dropdown">
                    ...
                    >
                      <button className="dropdown-item" onClick={exportToXLS}>
                        Download XLS
                      </button>
