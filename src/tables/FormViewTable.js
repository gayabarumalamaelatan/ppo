...
const FormTableView = ({ columns, data, columnVisibility, pageSize, totalItems, currentPage, onPageChange, formCode, menuName, primayKey, isLoadingTable, tableName }) => {
    ...
    const handleView = async (data) => {
        ...
        try {
            let response;
            if (tableName) {
                response = await axios.get(`${FORM_SERVICE_VIEW_DATA}?t=${tableName}&column=${primayKey}&value=${firstValue}`, { headers })
            } else {
                response = await axios.get(`${FORM_SERVICE_VIEW_DATA}?f=${formCode}&column=${primayKey}&value=${firstValue}`, { headers })
            }
    ...
    const renderPaginationItems = () => {
        ...
        if (pageCount <= 5) {
          ...
          if (startPage > 1) {
            paginationItems.push(
              <Pagination.Item key="start" onClick={() => handlePageChange(1)}>
                1
              </Pagination.Item>
            );
